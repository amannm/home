...
func init() {
	...
	rootCmd.PersistentFlags().DurationVar(&opts.Timeout, "timeout", 7*time.Second, "Request timeout (e.g. 2s, 500ms)")
	rootCmd.PersistentFlags().IntVar(&opts.Retries, "retries", 3, "Retry count on transient network errors")
